...
//    拿错类 begin() 会重试 10 次后返回 -2 CHIP_NOT_FOUND（看着像焊接问题）。
//    Ra-01S 是 **SX1268** → 必须用 SX1268 类。
class RadioLink {
 public:
  // 初始化 SX1268（参数来自 config.h：频率/带宽/SF/CR/同步字/功率）
  void begin();

  // 开机自检：读 0x0320 版本串 + 0x0740/0x0741 同步字 + GetDeviceErrors + 瞬时 RSSI，
  // 打印一行判据。版本串必须是 "SX1268"（同时证明 SPI 四线 + 供电 + 复位正常）。
  ...
  // TDMA MAC 直接访问 SX126x（基类指针；独占使用，非 TDMA 模式下勿与 send/poll 混用）
