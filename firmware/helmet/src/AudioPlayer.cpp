...
// 合成一段音效到 buf 的 p 位置：返回新位置
static uint32_t synthTone(int16_t *buf, uint32_t p, uint32_t n, float f0,
                          float f1, float amp) {
  float phase = 0;
  for (uint32_t i = 0; i < n; i++) {
    float t = (float)i / n;
    float f = f0 + (f1 - f0) * t;                 // 频率滑动（滑音）
    float env = sinf(3.14159265f * t);            // 正弦包络（无爆音）
...
static uint32_t synthNoise(int16_t *buf, uint32_t p, uint32_t n, float amp) {
  for (uint32_t i = 0; i < n; i++) {
    float t = (float)i / n;
    float env = expf(-4.0f * t);                  // 指数衰减（射击声）
    buf[p + i] =
...
// 播放：按音效类型即时合成到 _buf（合成 <1ms），随后 update() 喂 DMA
void AudioPlayer::play(Fx fx) {
  if (!_ok) return;
  _pos = 0;
  switch (fx) {
    case FX_SHOT:
      _len = synthNoise(_buf, 0, 1200, 16000);          // 150ms 噪声衰减
      break;
    case FX_HIT:
      _len = synthTone(_buf, 0, 800, 1200, 1200, 14000);   // 100ms 1.2kHz
      break;
    case FX_DEATH:
      _len = synthTone(_buf, 0, 2400, 800, 200, 16000);    // 300ms 下滑
      break;
    case FX_RESPAWN:
      _len = synthTone(_buf, 0, 2000, 300, 900, 15000);    // 250ms 上滑
      break;
    case FX_START: {
      uint32_t p = synthTone(_buf, 0, 800, 880, 880, 14000);
      _len = synthTone(_buf, p, 800, 1320, 1320, 14000);   // 双音
      break;
    }
    case FX_END:
      _len = synthTone(_buf, 0, 3200, 440, 440, 15000);    // 400ms 长音
      break;
    case FX_CLICK:
      _len = synthTone(_buf, 0, 320, 2000, 2000, 12000);   // 40ms 短音
      break;
    default:
      _len = 0;
      break;
