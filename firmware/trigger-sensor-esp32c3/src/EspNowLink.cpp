...
void EspNowLink::begin(const uint8_t peerMac[6]) {
  ...
  WiFi.mode(WIFI_STA);        // 仅初始化 2.4G 栈，不连接 AP
...
void EspNowLink::pushRx(uint8_t type, uint8_t d0) {
  ...
  if (next == _rxHead) return;   // 队列满：丢弃最旧前保护（直接丢弃新帧）
  _rx[_rxTail].type = type;
  _rx[_rxTail].d0 = d0;
  _rxTail = next;
}

void EspNowLink::printLocalMac() {
  uint8_t mac[6];
  esp_read_mac(mac, ESP_MAC_WIFI_STA);
  Serial.printf("[espnow] local MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
