...
class FrameRx {
 public:
  ...
  // 单/双通道：rx850 < 0 → 只用通道 0（38k 解码格），通道 1 不挂中断
  void begin(uint8_t rx940, int rx850 = -1);

  // 非阻塞轮询：任一通道成功返回 true；rawOut 非空时带回原始 40bit 帧(高32位起=b0)
  bool poll(LaserFrame &out, uint64_t *rawOut = nullptr);
