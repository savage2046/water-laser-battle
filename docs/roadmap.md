...
- **MAC**：多信道 TDMA（FDMA+TDMA）——单信道 LoRa 数学上无法容纳 50 台×200ms（单信道上限 17 台）
...
- **容量（2026-09-13 修订）**：**50 台 = 3 信道 × 17 时隙**（20 人 × 2 台[枪+头盔] = 40 台 + 10 台预留；
  原「200 设备 = 12 信道」方案作废）；单跳时延 ≤200ms（设备少时更低）；1km ✓
  （150dB 链路预算，15dBm 即够）
- **合规**：工信部 2019 年 52 号公告，470-510MHz 微功率 e.r.p. ≤50mW（17dBm）

### 0.2 本轮实现清单（代码状态：**已编译；单信道闭环已上板实测打通**，见 [docs/lora-联调记录-2026-09-13.md](lora-联调记录-2026-09-13.md)）
| 文件 | 内容 |
| --- | --- |
...
- T3b：多信道网关硬件（**3×SX1268** 板，槽位表 kRfSlots 填实际引脚；主控 ESP32-S3）
...
| **TDMA MAC（自适应超帧 30+10N ms / 信标 / 注册时隙 / 动态重排）** | ✅ **已上板实测**（单信道闭环打通并稳定 5 分钟，2026-09-13） |
...
| 一发一杀（网关唯一判定：跨通道远距 38kHz 优先 + 同通道光强度判定） | ✅ |
| 头盔：4 路双载波接收（波长统一 940nm）+ GPS（省电：电源开关+运动唤醒）+ 9 轴朝向 + 音效灯效 | ✅ |
...
### 激光链路
- **双载波同帧冗余**（波长已统一 940nm）：完整 40bit 帧（playerId+weapon+team+shotSeq+chk）
  双通道并行，任一通道解码成功即命中
- **近距通道软件可调功率（4 档，56kHz/940nm，历史命名 _850）**：默认档位限距 ≤20m
  （20m 外刻意不触发），升档（powerLevel 2/3）用于日光直射补偿与战术设定；
  20m+ 由远距（38kHz）透镜通道承担。选档由 IR_PWR_850_A/B（GPIO44/43，历史命名）
  2 位选档限流电阻组实现，见 hardware-design §6.5
- **双帧发射**（同 shotSeq 2 帧）+ 准直透镜（远距 38kHz）提升远距可靠性
...
- **仲裁规则**：跨通道 **远距（38kHz）优先**（近距 56kHz 先到被计后远距可抢占归属）；**同通道用光强度判定**（energy：枪端 30ms 双帧窗口，200=强光 100=弱光）
...
2. **GPIO12 是 strapping 引脚**（枪端近距通道发射（历史命名 _850）、头盔 IMU 电源都用它）——电路必须加 10kΩ 下拉
3. **MPU9250 磁力计未校准**——yaw 为相对值；绕 8 字采集极值做硬铁/软铁补偿（待办）
4. **近距通道（56kHz）射程上限 ~20m（默认档位设计意图）**——低档（powerLevel 0/1）
   电流校准在 20m 边界内；升档（2/3）按对局配置放宽（日光补偿/战术）。校准靶验证
   20m 外档 0/1 不触发（波长统一 940nm 后需按新电阻 100/100/51Ω 重标，见 hardware-design §6.5）
...
- [ ] 校准靶实测：10m（近距 56kHz 应触发）/ 20m（近距档 0/1 应**不**触发）/ 20m+（远距 38kHz 触发）
- [ ] 近距通道（56kHz/940nm）各档位限流电阻校准（档 0/1 以"20m 外不触发"为目标；档 2/3 记录放宽后边界，见 hardware-design §6.5）
- [ ] 磁力计 8 字校准（硬铁/软铁补偿），yaw 转绝对航向

### 中期（功能完善）
- [x] **无线升级 T1：单信道自适应 TDMA 固件**——`firmware/lib/tdma`
      **已实测：单信道闭环打通（枪端扫描→锁相→JOIN→ASSIGN→自身时隙发帧），
      网关 devs=1 稳定 5 分钟不掉（2026-09-13，详见 lora-联调记录-2026-09-13.md）**
      （TdmaMac/TdmaProto，FreeRTOS 任务）已接入枪/盔/网关三端：信标 + 广播下行窗 +
      设备时隙 + 注册时隙；开机负载均衡扫描自动分配信道（信标 N 最小者，平局随机，
      50 台/3 信道≈16-17 台/信道）；N 随活跃设备数自适应（超帧 30+10N ms）；
...
- [ ] **无线升级 T3b：多信道网关硬件**——**3×SX1268 多射频板**（槽位表 kRfSlots 填实际引脚）
      （目标 3 射频 = 50 台；渐进：1 → 3，对应 17 → 51 台）
      **算力核算见 docs/gateway-capacity.md**：CPU 够用（错峰+非阻塞 TX 后每核 ~40%），
      瓶颈在 RAM（任务栈须合并为 2-4 任务）与 GPIO（须换 ESP32-S3）；SPI 分 2 总线+互斥
- [ ] **无线升级 T4：多网关信标仲裁**——主/备信标网关切换 + 多网关共听 3 信道漫游
...
- [ ] 光强度升级：光电二极管 + ADC 实测光强（替换双帧窗口代理），同通道仲裁更精确
- [ ] 磁力计 8 字校准（硬铁/软铁补偿），yaw 转绝对航向
- [ ] 网关显示屏加音效/更多状态
- [ ] ctf 夺旗键改接（恢复夺旗模式）
...
> 模拟器已模拟：网关判定（newKill）+ 双载波 channel + 光强 energy + 多网关去重——
...
firmware/target/   校准靶（5×3 二维 38k 检测 + 中心解码）
